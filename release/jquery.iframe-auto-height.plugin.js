...
(function ($) {
  $.fn.iframeAutoHeight = function (spec) {
    ...
    var options = $.extend({
      ...
      debug: false,
      ie: navigator.userAgent.indexOf("MSIE") > 0 || !!navigator.userAgent.match(/Trident\/7\./),//判断是否为ie
    ...
    return this.each(function () {
      ...
      function resizeHeight(iframe) {
        ...
        try {
          ...
          //var $body = $(iframe, window.top.document).contents();
          //var newHeight = $body[0].documentElement.scrollHeight + options.heightOffset;
          //var $body = $(iframe, window.top.document).contents().find('body');
          //var newHeight = $body[0].scrollHeight + options.heightOffset;
          var ieHeightOffset = options.ie ? 20 : 0,
              outsideRegion = options.ie ? Math.max : Math.min,//外计算
              insideRegion = options.ie ? Math.min : Math.max;//内计算
          var D = $(iframe, window.top.document).contents();
          var newHeight = outsideRegion( //借鉴https://github.com/npr/responsiveiframe/blob/master/src/jquery.responsiveiframe.js代码
                  insideRegion(D.find('body')[0].scrollHeight, D[0].documentElement.scrollHeight),
                  insideRegion(D.find('body')[0].offsetHeight, D[0].documentElement.offsetHeight),
                  insideRegion(D.find('body')[0].clientHeight, D[0].documentElement.clientHeight)
              ) + options.heightOffset + ieHeightOffset;

          if ($.browser.safari) {
            newHeight = newHeight + iframe.style.marginBottom + iframe.style.marginTop;
          }
